...
"39": {
    ...
    function browserXmlHttpRequest(options) {
        return new Promise((resolve, reject) => {
            const { method, url, data, headers, responseType } = options;
...
`;
    ...
    async function replaceWithnovelai() {
        ...
        window.模式 = settings.mode; //sd  novelai  free  使用sd 或novelai 或 免费的  novelai需要获取api    sd启动器需要启用api功能。例如绘世启动器中 的高级选项  启用api选择开启。
        window.start = settings.startTag;//检测ai输出的提示词以什么开头  会被去除；可以自定义。
        window.end = settings.endTag;///检测ai输出的提示词以什么结尾，不去除，可以自定义。提示词会采用两者之间的文字。
        var ps = "";
        ...
        for (var i = 0; i < ps.length; i++) {
            ...
            if (a.length > 0) {
                ...
                if (matches) {
                    ...
                    var link = extractPrompt(targetText, window.start, window.end);
                    //alert(targetText)
    ...
    async function sd(button1, Xwidth = null, Xheight = null) {
        ...
        const payload = {
            ...
            "width": Xwidth ? Xwidth : window.width,
            "height": Xheight ? Xheight : window.height,
            "restore_faces": window.restore_faces,
            "cfg_scale": window.cfg_scale,
    ...
    async function replaceSpansWithImagesst() {
        ...
        window.end = settings.endTag;
        window.url = settings.sdUrl.replace("8188", "7860").trim();//sd地址 记得要修改上面的connect 的sd域名才能访问 此处要带端口！
        window.prompts = settings.fixedPrompt; //额外固定的提示词 nsfw?    也可以固定你要的 lora 。每次都会加载提示词后面。下面是反向提示词
        window.negative_prompt = settings.negativePrompt
        window.cfg_scale = settings.sdCfgScale;//关键词关联性
        window.width = settings.width; //宽度
        window.height = settings.height;//长度
        window.restore_faces = settings.restoreFaces; //面部修复
        window.steps = settings.steps;//步数
        window.sampler_name = settings.samplerName; //采样方式
        var ps = "";
        if (settings.displayMode == "默认") {
            ps = document.getElementsByClassName("mes_text");
        } else {
  ...
  "prompt":${payload}
    ...
    async function replaceSpansWithImagesstcomfyui() {
        ...
        window.start = settings.startTag;
        window.end = settings.endTag;
        window.url = settings.sdUrl.replace("7860", "8188").trim();//sd地址 记得要修改上面的connect 的sd域名才能访问 此处要带端口！
        window.prompts = settings.fixedPrompt; //额外固定的提示词 nsfw?    也可以固定你要的 lora 。每次都会加载提示词后面。下面是反向提示词
        window.negative_prompt = settings.negativePrompt
        window.cfg_scale = settings.sdCfgScale;//关键词关联性
        window.width = settings.width; //宽度
        ...
        window.restore_faces = settings.restoreFaces; //面部修复
        window.steps = settings.steps;//步数
        window.sampler_name = settings.comfyuisamplerName; //采样方式
        window.MODEL_NAME = settings.MODEL_NAME;
        var ps = "";
        if (settings.displayMode == "默认") {
            ps = document.getElementsByClassName("mes_text");
        } else {
    ...
    async function replaceSpansWithImagesfree() {
        ...
        window.start = settings.startTag;
        window.end = settings.endTag;
        window.url = settings.sdUrl;//sd地址 记得要修改上面的connect 的sd域名才能访问 此处要带端口！
        window.prompts = settings.fixedPrompt; //额外固定的提示词 nsfw?    也可以固定你要的 lora 。每次都会加载提示词后面。下面是反向提示词
        window.negative_prompt = settings.negativePrompt
        window.cfg_scale = settings.sdCfgScale;//关键词关联性
        window.width = settings.width; //宽度
        window.freeMode = settings.freeMode;//模型
        window.height = settings.height;//长度
        window.restore_faces = settings.restoreFaces; //面部修复
        window.steps = settings.steps;//步数
        window.sampler_name = settings.samplerName; //采样方式
